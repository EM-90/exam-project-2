...
const DateRangePicker: React.FC<DateRangePickerProps> = ({
  ...
  const dayClassName = (date: Date): string => {
    ...
    const className = isBookedDate
      ? "line-through rounded-md text-white text-lg"
      : "bg-green-100 rounded-md text-lg";
    return className;
  };
  ...
  return (
    <form
      className=" gap-3 flex flex-wrap md:flex-wrap sm:flex-nowrap lg:flex-nowrap justify-between bg-none sm:bg-skin-infoBg p-3"
    ...
    >
      ...
      {!isOwner && (
        <label className="lg:ml-2 w-full flex flex-col" htmlFor="guests">
          Guests
          <input
            type="number"
            id="guests"
          ...
          <PrimaryButton
            ...
            type="submit"
            className="mt-2 p-2 bg-skin-primary text-white"
          >
            Book Now
          </PrimaryButton>
          <p className="text-center pt-1">
            You have to be a registered customer to book a venue
          </p>
          <Link
            className="text-center underline text-blue-900 font-medium"
            to="/profile"
          >
