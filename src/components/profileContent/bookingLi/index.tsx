      ...
      <div className="flex flex-col sm:flex-row justify-between flex-wrap">
        <div className="flex flex-col md:flex-row gap-4">
          {venue.media && venue.media.length > 0 && (
            <img src={venue.media[0].url} alt={venue.media[0].alt || 'Venue image'} className=" w-full object-cover max-h-60 sm:min-h-32 sm:max-h-32 sm:min-w-44 sm:max-w-44 rounded-lg shadow-md" />
          )}
          <div className="flex flex-col md:ml-4">
            ...
            <div className="text-wrap max-w-50 pt-3">
              ...
              <p className="text-wrap text-skin-primary text-xl"><span>{location?.city}</span>, <span>{location?.address}</span></p>
            </div>
          </div>
        </div>
